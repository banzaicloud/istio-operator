...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.3.8 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.3.8` tag is the latest version of our operator to support Istio versions 1.3.x*
