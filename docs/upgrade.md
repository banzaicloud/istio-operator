...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.6.8, and we would like to upgrade our Istio components to Istio version 1.7.0. Here are the steps we need to perform to accomplish this with the operator:
...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.7.1 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.7.1` tag is the latest version of our operator to support Istio versions 1.7.x*
