...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.2.5, and we would like to upgrade our Istio components to Istio version 1.3.8. Here are the steps we need to perform to accomplish this with the operator:

1. Deploy a version of the operator which supports Istio 1.3.x
2. Apply a [Custom Resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) using Istio 1.3.8 components
...
To install Istio 1.3.8, first we need to check out the `release-1.3` branch of our operator (this branch supports the Istio 1.3.x versions):
...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.3.9 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.3.9` tag is the latest version of our operator to support Istio versions 1.3.x*
...
To deploy Istio 1.3.8 with its default configuration options, use the following command:
...
The `Istio` Custom Resource is showing `Available` in its status field, and the Istio components are now using `1.3.8` images:
...
$ kubectl describe istio -n istio-system istio | grep Image:
    ...
    Image:  docker.io/istio/proxy_init:1.3.8
    Image:                          docker.io/istio/sidecar_injector:1.3.8
      Image:                 docker.io/istio/install-cni:1.3.8

```

At this point, your Istio control plane is upgraded to Istio 1.3.8 and your BookInfo application should still be available at:
```bash
$ open http://$INGRESS_HOST/productpage
```
