...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.0.7, and we would like to upgrade our Istio components to Istio version 1.1.5. Here are the steps we need to perform to accomplish this with the operator:

1. Deploy a version of the operator which supports Istio 1.1.x
2. Apply a [Custom Resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) using Istio 1.1.5 components
...
To install Istio 1.1.5, first we need to check out the `release-1.1` branch of our operator (this branch supports the Istio 1.1.x versions):
```bash
$ git clone git@github.com:banzaicloud/istio-operator.git
...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.1.11 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.1.11` tag is the latest version of our operator to support Istio versions 1.1.x.*
...
To deploy Istio 1.1.5 with its default configuration options, use the following command:
...
The `Istio` Custom Resource is showing `Available` in its status field, and the Istio components are now using `1.1.5` images:
...
Spec:
  ...
  Exclude IP Ranges:
  Galley:
    Image:          docker.io/istio/galley:1.1.5
    Replica Count:  1
  Gateways:
    Egress:
      Max Replicas:   5
  ...
  Include IP Ranges:  *
  Mixer:
    Image:          docker.io/istio/mixer:1.1.5
    Max Replicas:   5
    Min Replicas:   1
    Replica Count:  1
  Mtls:             false
  Node Agent:
    Enabled:  false
    Image:    docker.io/istio/node-agent-k8s:1.1.5
  Outbound Traffic Policy:
    Mode:  ALLOW_ANY
  Pilot:
    Image:           docker.io/istio/pilot:1.1.5
    Max Replicas:    5
    Min Replicas:    1
    Replica Count:   1
    Trace Sampling:  1
  ...
  Sds:
    Enabled:  false
  Sidecar Injector:
    Image:                   docker.io/istio/sidecar_injector:1.1.5
    Replica Count:           1
...
At this point, your Istio control plane is upgraded to Istio 1.1.5 and your BookInfo application should still be available at:
```bash
$ open http://$INGRESS_HOST/productpage
```
