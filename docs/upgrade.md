...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.2.5, and we would like to upgrade our Istio components to Istio version 1.3.0. Here are the steps we need to perform to accomplish this with the operator:

1. Deploy a version of the operator which supports Istio 1.3.x
2. Apply a [Custom Resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) using Istio 1.3.0 components
...
To install Istio 1.3.0, first we need to check out the `release-1.3` branch of our operator (this branch supports the Istio 1.3.x versions):
...
The `Istio` Custom Resource is showing `Available` in its status field, and the Istio components are now using `1.3.0` images:
...
$ kubectl describe istio -n istio-system istio | grep Image:
    ...
    Image:                          docker.io/istio/sidecar_injector:1.3.0
      Image:                 docker.io/istio/install-cni:1.3.0

```

At this point, your Istio control plane is upgraded to Istio 1.3.0 and your BookInfo application should still be available at:
```bash
$ open http://$INGRESS_HOST/productpage
```
