...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.0.7, and we would like to upgrade our Istio components to Istio version 1.1.12. Here are the steps we need to perform to accomplish this with the operator:

1. Deploy a version of the operator which supports Istio 1.1.x
2. Apply a [Custom Resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) using Istio 1.1.12 components
...
#### Install Istio 1.1.12

To install Istio 1.1.12, first we need to check out the `release-1.1` branch of our operator (this branch supports the Istio 1.1.x versions):
```bash
$ git clone git@github.com:banzaicloud/istio-operator.git
...
To deploy Istio 1.1.12 with its default configuration options, use the following command:
...
The `Istio` Custom Resource is showing `Available` in its status field, and the Istio components are now using `1.1.12` images:
...
Spec:
  Auto Injection Namespaces:
    default
  Citadel:
    Image:                         docker.io/istio/citadel:1.1.12
    Replica Count:                 1
  ...
  Exclude IP Ranges:
  Galley:
    Image:          docker.io/istio/galley:1.1.12
    Replica Count:  1
  Gateways:
    Egress:
      Max Replicas:   5
  ...
  Include IP Ranges:  *
  Mixer:
    Image:          docker.io/istio/mixer:1.1.12
    Max Replicas:   5
    Min Replicas:   1
    Replica Count:  1
  Mtls:             false
  Node Agent:
    Enabled:  false
    Image:    docker.io/istio/node-agent-k8s:1.1.12
  Outbound Traffic Policy:
    Mode:  ALLOW_ANY
  Pilot:
    Image:           docker.io/istio/pilot:1.1.12
    Max Replicas:    5
    Min Replicas:    1
    Replica Count:   1
    Trace Sampling:  1
  Proxy:
    ...
    Image:             docker.io/istio/proxyv2:1.1.12
  Proxy Init:
    Image:  docker.io/istio/proxy_init:1.1.12
  Sds:
    Enabled:  false
  Sidecar Injector:
    Image:                   docker.io/istio/sidecar_injector:1.1.12
    Replica Count:           1
...
At this point, your Istio control plane is upgraded to Istio 1.1.12 and your BookInfo application should still be available at:
```bash
$ open http://$INGRESS_HOST/productpage
```
