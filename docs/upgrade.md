...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.4.14 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.4.14` tag is the latest version of our operator to support Istio versions 1.4.x*
