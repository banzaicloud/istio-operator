...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.5.7 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.5.7` tag is the latest version of our operator to support Istio versions 1.5.x*
...
$ kubectl describe istio -n istio-system istio | grep Image:
    ...
    Image:    docker.io/istio/node-agent-k8s:1.5.2
        Image:    docker.io/istio/node-agent-k8s:1.5.2
