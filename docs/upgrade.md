...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.5.2, and we would like to upgrade our Istio components to Istio version 1.6.0. Here are the steps we need to perform to accomplish this with the operator:

1. Deploy a version of the operator which supports Istio 1.6.x
2. Apply a [Custom Resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) using Istio 1.6.0 components
...
To install Istio 1.6.0, first we need to check out the `release-1.6` branch of our operator (this branch supports the Istio 1.6.x versions):
...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.6.1 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.6.1` tag is the latest version of our operator to support Istio versions 1.6.x*
...
To deploy Istio 1.6.0 with its default configuration options, use the following command:
...
$ kubectl describe istio -n istio-system istio | grep Image:
    Image:                         docker.io/istio/citadel:1.6.0
    Image:          docker.io/istio/galley:1.6.0
    Image:    docker.io/istio/node-agent-k8s:1.6.0
        Image:    docker.io/istio/node-agent-k8s:1.6.0
    Image:          coredns/coredns:1.6.2
    Plugin Image:   docker.io/istio/coredns-plugin:0.2-istio-1.1
    Image:          docker.io/istio/mixer:1.6.0
    Image:    docker.io/istio/node-agent-k8s:1.6.0
