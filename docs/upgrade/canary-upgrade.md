...
Let us suppose that we have an Istio 1.9 control plane running in our cluster and we would like to upgrade to an Istio 1.10 control plane.
...
1. Deploy an Istio 1.10 control plane *next* to the Istio 1.9 control plane
1. Migrate the data plane *gradually* to the new control plane
1. When migration is fully finished, uninstall the Istio 1.9 control plane
...
So our starting point is a running Istio operator and Istio 1.9 control plane, with applications using that control plane version.

1. Deploy another Istio operator (alongside the previous one), which has Istio 1.10 support
1. Apply a new Istio CR with Istio version 1.10 and let the operator reconcile an Istio 1.10 control plane

   It is recommended that you turn off gateways for the new control plane and migrate the 1.9 gateway pods to the 1.10 control plane. That way the ingress gateway's typically LoadBalancer-type service will not be recreated, neither will the cloud platform-managed LoadBalancer, and hence the IP address won't change either.
...
1. Migrate gateways to use the new Istio 1.10 control plane
...
1. Migrate data plane applications
  ...
  During the data plane migration, it is important to keep in mind that the two Istio control planes share trust (because they use the same certificates for encrypted communication). Therefore, when an application pod that still uses Istio's 1.9 control plane calls another pod that already uses Istio's 1.10 control plane, the encrypted communication will succeed because of that shared trust.
...
1. Delete the Istio 1.9 control plane

   Once the migration is finished, and you've made sure that your applications are working properly in conjunction with the new control plane, the older Istio 1.9 control plane can be safely deleted.
...
First, we'll deploy an Istio 1.9 control plane with the Istio operator and two demo applications in separate namespaces.
Then, we'll deploy an Istio 1.10 control plane alongside the other control plane and migrate the demo applications to the new control plane gradually.
...
When the data plane migration is finished, we'll delete the Istio 1.9 control plane and operator.
...
1. Deploy an Istio operator version, which can install an Istio 1.9 control plane.
   ...
   $ helm install istio-operator-v19x --create-namespace --namespace=istio-system --set-string operator.image.tag=0.9.5 --set-string istioVersion=1.9 banzaicloud-stable/istio-operator
   ```
...
1. Check that the Istio 1.9 control plane is deployed.

   ```bash
   $ kubectl get po -n=istio-system
   NAME                                                      READY   STATUS    RESTARTS   AGE
   istio-istio-sample-v19x-ingressgateway-55b89d99d7-4m884   1/1     Running   0          17s
   istio-operator-v19x-0                                     2/2     Running   0          57s
   istiod-istio-sample-v19x-5865cb6547-zp5zh                 1/1     Running   0          29s
   ```
...
1. Add those namespaces to the mesh.
   ...
   ```bash
   $ kubectl label ns demo-a istio.io/rev=istio-sample-v19x.istio-system
   $ kubectl label ns demo-b istio.io/rev=istio-sample-v19x.istio-system
   ```
...
1. Make sure that the namespaces are labeled for sidecar injection.
   ...
   NAME     STATUS   AGE     REV
   demo-a   Active   2m11s   istio-sample-v19x.istio-system
   demo-b   Active   2m9s    istio-sample-v19x.istio-system
   ```
...
1. Deploy an Istio operator version, which can install an Istio 1.10 control plane.
   ...
   ```bash
   $ helm install istio-operator-v110x --create-namespace --namespace=istio-system --set-string operator.image.tag=v0.10.4 banzaicloud-stable/istio-operator
   ```
...
1. Apply an `Istio` Custom Resource and let the operator reconcile the Istio 1.10 control plane.

   ```bash
   $ kubectl apply -n istio-system -f https://raw.githubusercontent.com/banzaicloud/istio-operator/release-1.10/config/samples/istio_v1beta1_istio.yaml
   ```

1. Check that the Istio 1.10 control plane is also deployed.

   ```bash
   $ kubectl get po -n=istio-system
   NAME                                                          READY   STATUS    RESTARTS   AGE
   istio-istio-sample-v19x-ingressgateway-55b89d99d7-4m884       1/1     Running   0          6m38s
   istio-operator-v19x-0                                         2/2     Running   0          7m18s
   istio-operator-v110x-0                                        2/2     Running   0          76s
   istiod-istio-sample-v110x-676fc6d449-9jwfj                    1/1     Running   0          10s
   istiod-istio-sample-v19x-7dbdf4f9fc-bfxhl                     1/1     Running   0          6m32s
   ```
...
1. Change the ingress gateway so that it utilizes the new Istio 1.10 control plane.

   ```bash
   $ kubectl patch mgw -n istio-system istio-ingressgateway-istio-sample-v19x --type=json -p='[{"op": "replace", "path": "/spec/istioControlPlane/name", "value": "istio-sample-v110x"}]'
   ```
...
1. Label the first namespace so that all workloads there utilize the new control plane.
   ...
   ```bash
   $ kubectl label ns demo-a istio-injection- istio.io/rev=istio-sample-v110x.istio-system --overwrite
   ```
...
1. Make sure that the labeling is correct.
   ...
   demo-a   Active   12m                     istio-sample-v110x.istio-system
   ```
...
1. Make sure that the new 1.10 sidecar proxy is used for the new pod.
...
Remember that the pod(s) in the `demo-a` namespace are already on the Istio 1.10 control plane, but the pod in `demo-b` is still using the Istio 1.9 version.
...
1. Label the second namespace so that all workloads there utilize the new control plane.
   ...
   ```bash
   $ kubectl label ns demo-b istio-injection- istio.io/rev=istio-sample-v110x.istio-system --overwrite
   ```
...
1. Make sure that the labeling is correct.
   ...
   NAME     STATUS   AGE   ISTIO-INJECTION   REV
   demo-b   Active   19m                     istio-sample-v110x.istio-system
   ```
...
1. Make sure that now the new 1.10 sidecar proxy is used for the new pod.
...
When the data plane is fully migrated to the 1.10 version and you made sure that it works as expected, we can delete the "old" Istio 1.9 control plane.

1. Delete the 1.9 `Istio` Custom Resource to delete the Istio 1.9 control plane.

   ```bash
   $ kubectl delete -n istio-system -f https://raw.githubusercontent.com/banzaicloud/istio-operator/release-1.9/config/samples/istio_v1beta1_istio.yaml
   ```

1. Uninstall the Istio operator for version 1.9.

   > :warning: If you installed the Istio operator chart for Istio 1.9 with Helm2, then first upgrade that chart with Helm version 3.1.0, otherwise your CRDs will be deleted!
   > ```bash
   > $ helm upgrade istio-operator-v19x --namespace=istio-system --set-string operator.image.tag=0.9.5 --set-string istioVersion=1.9 banzaicloud-stable/istio-operator
   > ```
   ...
   ```bash
   $ helm uninstall -n=istio-system istio-operator-v19x
   ```
