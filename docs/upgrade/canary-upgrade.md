...
Let us suppose that we have an Istio 1.7 control plane running in our cluster and we would like to upgrade to an Istio 1.9 control plane.
...
1. Deploy an Istio 1.9 control plane *next* to the Istio 1.7 control plane
...
1. Migrate data plane applications
  ...
  During the data plane migration, it is important to keep in mind that the two Istio control planes share trust (because they use the same certificates for encrypted communication). Therefore, when an application pod that still uses Istio's 1.7 control plane calls another pod that already uses Istio's 1.9 control plane, the encrypted communication will succeed because of that shared trust.
...
1. Deploy an Istio operator version, which can install an Istio 1.7 control plane.
   ...
   $ helm install istio-operator-v17x --create-namespace --namespace=istio-system --set-string operator.image.tag=0.8.3 --set-string istioVersion=1.7 banzaicloud-stable/istio-operator
   ```
...
1. Deploy an Istio operator version, which can install an Istio 1.9 control plane.
   ...
   ```bash
   $ helm install istio-operator-v18x --create-namespace --namespace=istio-system --set-string operator.image.tag=0.9.0 banzaicloud-stable/istio-operator
   ```
...
Remember that the pod(s) in the `demo-a` namespace are already on the Istio 1.9 control plane, but the pod in `demo-b` is still using the Istio 1.7 version.
...
When the data plane is fully migrated to the 1.9 version and you made sure that it works as expected, we can delete the "old" Istio 1.7 control plane.
...
1. Uninstall the Istio operator for version 1.7.
   ...
   > ```bash
   > $ helm upgrade istio-operator-v17x --namespace=istio-system --set-string operator.image.tag=0.8.3 --set-string istioVersion=1.7 banzaicloud-stable/istio-operator
   > ```
