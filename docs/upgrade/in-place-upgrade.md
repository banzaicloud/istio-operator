...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.9.9 --set-string istioVersion=1.9.8 banzaicloud-stable/istio-operator
```
