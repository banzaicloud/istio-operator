...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.8.0 --set-string istioVersion=1.8.1 banzaicloud-stable/istio-operator
```
