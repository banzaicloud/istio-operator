...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.8.10 --set-string istioVersion=1.8.4 banzaicloud-stable/istio-operator
```
