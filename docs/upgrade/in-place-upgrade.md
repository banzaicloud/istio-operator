...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=v0.9.11 --set-string istioVersion=1.9.8 banzaicloud-stable/istio-operator
```

*Note: As of now, the `v0.9.11` tag is the latest version of our operator to support Istio versions 1.9.x*
