...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.10.1 --set-string istioVersion=1.10.2 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.10.1` tag is the latest version of our operator to support Istio versions 1.10.x*
