...
Let us suppose that we have a [Kubernetes](https://kubernetes.io/) cluster with Istio 1.7.4, and we would like to upgrade our Istio components to Istio version 1.8.4. Here are the steps we need to perform to accomplish this with the operator:

1. Deploy a version of the operator which supports Istio 1.8.x
2. Apply a [Custom Resource](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/) using Istio 1.8.4 components
...
To install Istio 1.8.4, first we need to check out the `release-1.8` branch of our operator (this branch supports the Istio 1.8.x versions):
...
```bash
$ helm repo add banzaicloud-stable https://kubernetes-charts.banzaicloud.com
$ helm upgrade istio-operator --install --namespace=istio-system --set-string operator.image.tag=0.8.12 --set-string istioVersion=1.8.4 banzaicloud-stable/istio-operator
```

*Note: As of now, the `0.8.12` tag is the latest version of our operator to support Istio versions 1.8.x*
...
To deploy Istio 1.8.4 with its default configuration options, use the following command:
...
The `Istio` Custom Resource is showing `Available` in its status field, and the Istio components are now using `1.8.4` images:

```bash
$ kubectl describe istio -n istio-system istio | grep Image:
    Image:                         docker.io/istio/citadel:1.8.4
    Image:          docker.io/istio/galley:1.8.4
    Image:    docker.io/istio/node-agent-k8s:1.8.4
        Image:    docker.io/istio/node-agent-k8s:1.8.4
    Image:          coredns/coredns:1.8.4
    Plugin Image:   docker.io/istio/coredns-plugin:0.2-istio-1.1
    Image:          docker.io/istio/mixer:1.8.4
    Image:    docker.io/istio/node-agent-k8s:1.8.4
    Image:          docker.io/istio/pilot:1.8.4
    Image:             docker.io/istio/proxyv2:1.8.4
    Image:  docker.io/istio/proxyv2:1.8.4
    Image:                          docker.io/istio/sidecar_injector:1.8.4
      Image:                 docker.io/istio/install-cni:1.8.4
```

At this point, your Istio control plane is upgraded to Istio 1.8.4 and your BookInfo application should still be available at:
```bash
$ open http://$INGRESS_HOST/productpage
```
